...
export async function runAgentStream(input: RunAgentInput) {
  ...
  const system = [
    ...
    "2) Create real files and folders on this PC with create_directory, create_project_folder, write_file, and scaffold_project — never only describe them. Always report the absolutePath from tool results.",
    ...
    `Helix Projects root (for create_project_folder): see Documents/Helix/Projects`,
    `Current project map:\n${projectMap.summary}`,
